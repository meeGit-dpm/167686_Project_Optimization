...
# Min route time 
def compute_min_time(customers):
    time = 0
    current_pos = 0
    total_time = 0
    for next_pos in customers: 
        time += matrix[customers[current_pos]][customers[next_pos]]
        total_time += matrix[customers[current_pos]][customers[next_pos]]

        if time < e[customers[next_pos]]:
            time = e[customers[next_pos]]
        if time > l[customers[next_pos]]:
            time = float('inf')
        
        time += d[customers[current_pos]] 
        current_pos = next_pos

    total_time += matrix[customers[-2]][0]
...
def hill_climbing(routes, max_iter=10000):
    best_routes = routes
    best_cost = compute_min_time(routes)

    for _ in range(max_iter):
        # Try Swap
        neighbor = generate_neighbor_swap(best_routes)
        cost = compute_min_time(neighbor)
        if cost < best_cost:
            best_routes, best_cost = neighbor, cost
            continue

        # Try Swap Multiple
        neighbor = generate_neighbor_swap_multiple(best_routes)
        cost = compute_min_time(neighbor)
        if cost < best_cost:
            best_routes, best_cost = neighbor, cost
            continue
