...
\begin{titlepage}
    ...
    \begin{center}
        ...
        {\Large
            Bui Dang Quang \\[0.2cm]
            Dinh Phuong Mai \\[0.2cm]
            Do Anh Minh \\[0.2cm]
            Ngo Nam Nhat Minh\\[0.2cm]
        }
    \end{center}
...
\subsection{Background}
Constraint Programming with \texttt{AddCircuit} provides mathematically guaranteed solutions for small instances of the Balanced Staff Routing problem. Compared to the original MTZ-based formulation, the key improvements are: using \texttt{BoolVar} instead of \texttt{IntVar(0,1)} for routing variables, tightening the domain bounds of $y[k]$ and $z$, and replacing three separate constraint groups (flow conservation, depot constraints, and MTZ subtour elimination) with a single \texttt{AddCircuit} call per technician. The rest of the model — the visit-once constraint, workload calculation, and route reconstruction — is unchanged from the original.
...
\begin{itemize}
    ...
    \item $\text{self\_loop}[i][k]$ (\texttt{BoolVar}): Equals 1 if technician $k$ does NOT visit customer $i$. This auxiliary variable is required by \texttt{AddCircuit} to allow a circuit to skip certain nodes. It replaces the MTZ ordering variable $u_{i,k}$ used in the original formulation.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Subtour elimination:} no disconnected sub-cycles (replaces the MTZ constraints $u_{i,k} - u_{j,k} + N \cdot x_{i,j,k} \le N-1$ and the auxiliary variable $u_{i,k}$)
    \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{AddCircuit replaces MTZ but does not reduce variable count}\newline
    The key improvement in this implementation over the original code is replacing the MTZ subtour elimination constraints (which required $N^2 \times K$ additional integer constraints and the ordering variables $u_{i,k}$) with a single \texttt{AddCircuit} call per technician. This makes the model significantly faster for small instances by giving CP-SAT a more efficient internal propagator for circuit constraints. However, it does not reduce the number of $x[i][j][k]$ variables, which remains $O(K \times N^2)$. The memory bottleneck is therefore unchanged.
